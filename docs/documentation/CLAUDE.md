...
| Inference sampling parameters | `MAX_TOKENS_ANALYZE` 160, `MAX_TOKENS_INSPECT` 256, `MAX_TOKENS_EVAL` 200, `temperature` 0.3, `repeat_penalty` 1.15 — LOCKED Phase 16 | Passed explicitly instead of inheriting handler defaults. Caps set above observed maximums after the eval gate caught truncation at lower values (Inspect cut mid-item at 160, eval Text Reading cut mid-phrase at 120). Treat them as worst-case ceilings, not a speedup: measurement showed no average latency win because the model naturally stops between 30 and 90 tokens. All env overridable. |
...
- ~~Inference parameter tuning~~ — RESOLVED in Phase 16. The eval gate ran on the Jetson and the values are now locked; see the Active Decisions table above and the Phase 16 gate record in BRIEFING.md.
...
- **max_tokens is a ceiling, not a speedup.** The Phase 16 premise was that `max_tokens` is the single biggest latency lever. Measured on the Jetson, that did not hold. Moondream2 naturally stops between 30 and 90 tokens on these prompts, so the caps almost never engage, and a before/after eval showed no real average latency change once scene variation and a first-prompt CUDA warmup outlier are accounted for. Worse, caps set aggressively low silently truncated answers mid-sentence. Keep caps as worst-case ceilings set comfortably above observed maximums, and do not expect them to make typical responses faster.
- **Latency is mostly fixed image prefill, not generation.** A typical response costs roughly 3.5s of image prefill plus generation at about 17 to 18 tokens per second. A 50-token answer is therefore dominated by prefill, which is why shortening the answer buys so little. This is what makes Task 6's `n_batch` experiment (the ~729-position image prefill currently splitting across two batches at `n_batch` 512) the more promising optimisation.
- **flash_attn=True is the one real performance win.** Benchmarked on the Orin Nano (sm87) against a fixed image: prefill 3.05s to 2.18s (28.7 percent) and end to end 7.34s to 6.28s (14.4 percent). The runtime logs "flash attention is enabled", so the kernels genuinely engage rather than falling back. `n_batch=1024` and `n_threads=6` were both measured and reverted: the first is negligible alone and adds nothing once flash attention is on, the second is a null result because the GPU does the work. Gated behind `FLASH_ATTN` in config.py in case a future llama.cpp build misbehaves.
- **Loading a second `Llama` model in the same process aborts with a CUDA error.** Any A/B test or hot-swap must run each configuration in its own process. Separately, loading a second model while the server already holds one exhausts the 8GB unified memory (MemAvailable drops to about 1.3GB) and the load dies. Stop the server before running any benchmark that loads the model.
- **Benchmark the right variable.** The eval suite runs against a live camera, so scene variation swamps small latency effects and makes before/after comparisons unreliable. For single-variable performance work, use a fixed saved frame, a fixed prompt, and `max_tokens=1` to isolate prefill, with several samples per configuration. That instrument cleanly separated flash_attn from baseline where the eval suite could not.
- **Only one process can hold the camera.** V4L2 allows a single opener of `/dev/video0`, so a second server instance fails at startup with "Could not open camera at device index 0" and the lifespan aborts. When that error appears, check for an already-running server (`fuser -v /dev/video0`) before suspecting the camera or the code.
- **Native video controls cannot be repositioned.** Safari renders the `<video controls>` control bar in its own shadow DOM and on iOS occupies both top corners, so any overlaid button on the video surface risks colliding with it. The fix is structural: put the app's own controls outside the video element rather than trying to dodge the browser's.
- **`pkill -f "uvicorn server:app"` can kill its own shell.** When run inside a command whose own command line contains that pattern, `pkill`/`pgrep -f` matches the invoking shell and it kills itself before doing anything useful (observed as an immediate exit 144 with no log). Use a bracket pattern like `pkill -f "[u]vicorn server:app"`, or kill by PID.
...
| Phase 16 — Finalizing & Hardening | ✅ Complete — all tasks done and gated on hardware (July 21, 2026) |
...
Phase 16 (Finalizing & Hardening) started July 20, 2026. Renamed from "Future Implementations." The code was written without the camera attached; the gates were then run on the Jetson with the BRIO on July 21, 2026, and Tasks 1, 2, 4, 5, and 7 passed. Task 2's gate failed on the first attempt by catching silently truncated answers, which forced the token caps upward, and its measured latency benefit turned out to be a null result. Task 3's fresh-clone gate then passed as well: a throwaway clone with models symlinked ran the README end to end, booted, generated its own secrets and admin password, and logged in, confirming the repo is genuinely self-sufficient. Task 6 then ran as well: flash_attn was adopted for a 14 percent end-to-end gain, while n_batch and n_threads were measured and reverted. Phase 16 is complete. Step 0 restored the missing config.py as a committed secrets-from-env file. Task 1 added camera staleness detection and frame_hash duplicate-response diagnostics. Task 2 passes explicit inference parameters (values provisional pending the eval gate). Task 3 added the unauthenticated `/health` endpoint and fixed the launch.sh 401 wait; the interface is now web-only. Task 4 removed passlib (broken against bcrypt 5.x) in favor of direct bcrypt, moved main.py and pipeline.py to legacy/, and removed the racy session_log.json write. Task 5 fixed the sync MJPEG generator, autoscan interval math, and eval frame-capture ordering. Task 7 added a CSS-only responsive layer for phones and tablets. Task 8 stripped phase labels from source. See BRIEFING.md for per-task gate status and the full pending-gate checklist.
